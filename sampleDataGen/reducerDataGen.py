    ...
    for (low_d1_in, high_d1_in), cnt in zip(bins_d1_in, counts):
        ...
        for d1_cm_val in D1_cm_samples:
            ...
            if d2_cm == -1.0:
                ...
                # 또는 다른 D2 생성 전략을 사용할 수 있습니다 (예: D1에 더 가깝게 D2를 설정).
...
def main():
    parser = argparse.ArgumentParser(
        description="Reducer 샘플 데이터 생성 (D1/D2: inch, Length: cm). D1_cm > D2_cm, D2_cm < 0.7*D1_cm"
    )
    parser.add_argument("n", type=int,
                        help="생성할 전체 샘플 수 (예: 1000)")
    parser.add_argument("-o","--output", default="reducer_samples.xlsx",
                        help="출력 엑셀 파일명")
    ...
    print(f"완료: '{args.output}'에 {len(df)}개의 Reducer 샘플을 저장했습니다.")
