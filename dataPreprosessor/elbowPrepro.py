...
def parse_elbow_file(path: Path, start_sample_id: int, model_data_blocks: list):
    ...
    while i < len(lines):
        if re.match(r'Data for Conductance\s+\d+', lines[i].strip()):
            ...
            # Search for header in the next few lines
            for j in range(i + 1, min(i + 4, len(lines))):
                m_geom = re.search(r'\d+\s+ELBOW\(s\),\s*(\d+)\s+Degrees,\s*D=\s*([\d\.E+-]+)\s*Cm', lines[j])
                if m_geom:
                    angle_deg, diameter_cm = int(m_geom.group(1)), float(m_geom.group(2))
                    header_found = True
                    i = j
                    break
            ...
            additional_data = model_data_blocks[current_sample_id_in_file] if current_sample_id_in_file < len(model_data_blocks) else {col: None for col in NEW_COLUMNS_ELBOW}
            ...
            while data_cursor < len(lines) and not re.match(r'Data for Conductance\s+\d+', lines[data_cursor].strip()):
                ...
                if m_data:
                    row_data = {
                        "SampleID": assigned_sample_id, "Diameter_cm": round(diameter_cm, 4),
                        "BendAngle_deg": angle_deg, "Quantity": quantity,
                        "Pressure_Torr": float(m_data.group(1)), "Conductance_L_per_min": float(m_data.group(2))
                    }
...
def run(input_path_str, output_file):
    """Parses VACTRAN TXT output files for elbows and generates a final CSV."""
        ...
        if model_file_path.exists():
            with model_file_path.open('r', encoding='utf-8') as mf:
                ...
                for section_match in elbow_block_sections:
                    if "Viscous K factor" in section_match.group(1):
                         current_file_model_data_blocks.append(parse_elbow_block_data(section_match.group(1)))
...
def main():
    ...
    parser.add_argument('-o', '--output', default='elbow_output.csv', help='Output CSV filename.')
